...
export async function enqueueEmbedBulk(
  ...
  // Pick sources that still have un-embedded chunks.
  const res = await query<{ id: string; missing: string }>(`
    SELECT s.id,
           (SELECT COUNT(*) FROM knowledge_chunks c
            WHERE c.source_id = s.id AND c.embedding IS NULL)::text AS missing
    FROM   knowledge_sources s
    WHERE  s.tenant_id = $1 AND s.status = 'ready' AND s.chunk_count > 0
  ...
  return { enqueued, sourcesPending }
}
