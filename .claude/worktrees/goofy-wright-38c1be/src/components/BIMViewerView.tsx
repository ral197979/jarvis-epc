...
export function BIMViewerView({ policy }: BIMViewerViewProps) {
  ...
  // Fetch APS token via server-side proxy (never exposes client_secret to browser)
  useEffect(() => {
    fetch('/api/v1/bim/aps-token', { method: 'POST' })
      .then(r => r.ok ? r.json() : null)
      .then(d => { if (d?.access_token) setApsToken(d.access_token) })
      .catch(() => {})
  }, [])
          ...
          {selected ? (
            <div>
              ...
              <div style={{ height: 420, background: '#1a1a1a', border: '1px solid var(--jarvis-bd)', borderRadius: 4, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#888' }}>
                {selected.document_id && apsToken ? (
                  <iframe
                    src={`https://xeokit.github.io/xeokit-sdk/examples/loading/#loading_IFC_Duplex?src=${encodeURIComponent(`/api/v1/documents/${selected.document_id}/file`)}&token=${encodeURIComponent(apsToken)}`}
                  ...
                  <div style={{ textAlign: 'center' }}>
                    <div>3D viewer ready{apsToken ? ' (APS token active)' : ''}.</div>
