/**
 * JARVIS EPC — NeView  ·  Network Engineering
 */
...
export interface NeViewProps { policy?: Partial<PolicyConfig> }

export function NeView({ policy: _p }: NeViewProps) {
  ...
  return (
    <div role="main" aria-label="Network Engineering">
      ...
      <div style={{ marginBottom: 10 }}>
        <input className="jarvis-input" type="search" placeholder="Search network engineering…"
          value={search} onChange={e => setSearch(e.target.value)} aria-label="Search Network Engineering" />
      </div>
      {filtered.length === 0 ? (
        <div className="jarvis-empty" role="status">
          <span className="jarvis-empty-icon">🌐</span>
          <span>{search ? 'No items match' : 'No network deliverables'}</span>
        </div>
      ) : (
        <div className="jarvis-scroll-y jarvis-max-h-lg">
          <table className="jarvis-table" aria-label="Network Engineering">
