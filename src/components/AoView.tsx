...
export interface AoViewProps { policy?: Partial<PolicyConfig> }

export function AoView({ policy: _p }: AoViewProps) {
  ...
  return (
    <div role="main" aria-label="Architecture Overview">
      ...
      <div style={{ marginBottom: 10 }}>
        <input className="jarvis-input" type="search" placeholder="Search architecture overview…"
          value={search} onChange={e => setSearch(e.target.value)} aria-label="Search Architecture Overview" />
      </div>
      {filtered.length === 0 ? (
        <div className="jarvis-empty" role="status">
          <span className="jarvis-empty-icon">🏛️</span>
          <span>{search ? 'No items match' : 'No architecture deliverables'}</span>
        </div>
      ) : (
        <div className="jarvis-scroll-y jarvis-max-h-lg">
          <table className="jarvis-table" aria-label="Architecture Overview">
