/**
 * JARVIS EPC — WnView  ·  Work Notes
 */
import React, { useState } from 'react'
import { useBizStore } from '../modules/biz/store'
import type { PolicyConfig } from '../modules/biz/dispatch'

export interface WnViewProps { policy?: Partial<PolicyConfig>; projectId?: string }

export function WnView({ policy: _p, projectId }: WnViewProps) {
  ...
  return (
    <div role="main" aria-label="Work Notes">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
        <h3 className="jarvis-heading" style={{ margin: 0 }}>Work Notes</h3>
        <span className="jarvis-small" style={{ color: 'var(--jarvis-ts)' }}>{filtered.length} records</span>
      </div>
      <div style={{ marginBottom: 10 }}>
        <input className="jarvis-input" type="search" placeholder="Search work notes…" value={search} onChange={e => setSearch(e.target.value)} aria-label="Search Work Notes" />
      </div>
      {filtered.length === 0 ? (
        <div className="jarvis-empty"><span className="jarvis-empty-icon">📝</span><span>{search ? 'No notes match' : 'No work notes recorded'}</span></div>
      ) : (
