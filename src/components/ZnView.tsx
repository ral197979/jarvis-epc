/**
 * JARVIS EPC — ZnView  ·  Zone Notes
 */
import React, { useState } from 'react'
import { useBizStore } from '../modules/biz/store'
import type { PolicyConfig } from '../modules/biz/dispatch'

export interface ZnViewProps { policy?: Partial<PolicyConfig>; projectId?: string }

export function ZnView({ policy: _p, projectId }: ZnViewProps) {
  ...
  return (
    <div role="main" aria-label="Zone Notes">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
        <h3 className="jarvis-heading" style={{ margin: 0 }}>Zone Notes</h3>
        <span className="jarvis-small" style={{ color: 'var(--jarvis-ts)' }}>{filtered.length} records</span>
      </div>
      <div style={{ marginBottom: 10 }}>
        <input className="jarvis-input" type="search" placeholder="Search zone notes…" value={search} onChange={e => setSearch(e.target.value)} aria-label="Search Zone Notes" />
      </div>
      {filtered.length === 0 ? (
        <div className="jarvis-empty"><span className="jarvis-empty-icon">🗂️</span><span>{search ? 'No notes match' : 'No zone notes recorded'}</span></div>
      ) : (
