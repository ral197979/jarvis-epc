/**
 * JARVIS EPC — EeView  ·  Electrical Engineering
 */
...
export interface EeViewProps { policy?: Partial<PolicyConfig> }

export function EeView({ policy: _p }: EeViewProps) {
  ...
  return (
    <div role="main" aria-label="Electrical Engineering">
      ...
      <div style={{ marginBottom: 10 }}>
        <input className="jarvis-input" type="search" placeholder="Search electrical engineering…"
          value={search} onChange={e => setSearch(e.target.value)} aria-label="Search Electrical Engineering" />
      </div>
      {filtered.length === 0 ? (
        <div className="jarvis-empty" role="status">
          <span className="jarvis-empty-icon">⚡</span>
          <span>{search ? 'No items match' : 'No electrical deliverables'}</span>
        </div>
      ) : (
        <div className="jarvis-scroll-y jarvis-max-h-lg">
          <table className="jarvis-table" aria-label="Electrical Engineering">
