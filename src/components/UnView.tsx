...
export function UnView({ policy: pProp, onToast }: UnViewProps) {
  ...
  return (
    <div role="main" aria-label="Unit Notes">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <h3 className="jarvis-heading" style={{ margin: 0 }}>Unit Notes</h3>
      ...
      {filtered.length === 0 ? (
        <div className="jarvis-empty"><span className="jarvis-empty-icon">📝</span><span>{search ? 'No notes match' : 'No unit notes recorded yet'}</span></div>
      ) : (
