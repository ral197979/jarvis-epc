/**
 * JARVIS EPC — AnView  ·  Analysis & Notes
 */
import React, { useState } from 'react'
import { useBizStore } from '../modules/biz/store'
import { createDispatch, type PolicyConfig } from '../modules/biz/dispatch'

export interface AnViewProps { policy?: Partial<PolicyConfig>; onToast?: (msg: string, type: string) => void }
const DEF: PolicyConfig = { writesEnabled: false, chatEnabled: false, exportsEnabled: false, activeRole: 'viewer' }

export function AnView({ policy: pProp, onToast }: AnViewProps) {
  ...
  return (
    <div role="main" aria-label="Analysis & Notes">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <h3 className="jarvis-heading" style={{ margin: 0 }}>Analysis & Notes</h3>
      ...
      {filtered.length === 0 ? (
        <div className="jarvis-empty"><span className="jarvis-empty-icon">📊</span><span>{search ? 'No notes match' : 'No analysis & notes recorded yet'}</span></div>
      ) : (
